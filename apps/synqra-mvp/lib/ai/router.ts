...
export async function route(task: AITask): Promise<RoutingDecision> {
  ...
  const tokenLimit = TOKEN_LIMITS[selectedModel as keyof typeof TOKEN_LIMITS] || 1000;
...
export async function executeTask(task: AITask): Promise<any> {
  ...
  for (const step of decision.pipeline || []) {
    ...
    switch (action) {
      ...
      case 'execute':
        // Main model execution
        response = await callModel(model as ModelProvider, response || task.input, task.systemPrompt, TOKEN_LIMITS[model as keyof typeof TOKEN_LIMITS] || 1000);
        break;
