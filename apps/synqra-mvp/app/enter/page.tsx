...
function EntranceInner() {
  ...
  const accessIdentityAsset = useMemo(
    () => resolveIdentityAssetForSurface("access", ["system_seal", "monogram_stamp"]),
    []
  );
  const showSystemSeal = accessIdentityAsset === "system_seal";
  const showMonogram = accessIdentityAsset === "monogram_stamp";
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    if (provider !== "google") {
      setOAuthError("Apple sign-in is not configured");
      return;
    }
    ...
    try {
      ...
      // Keep timeout active until browser navigates away.
    }
  };
          ...
          <div className="relative p-8 md:p-10">
            <div className="flex items-center justify-between gap-6">
              ...
              {status !== "idle" && !isTransitioning ? (
                <div
                  className={`font-mono text-[0.72rem] uppercase tracking-[0.18em] ${status === "accepted" ? "text-noid-gold" : "text-noid-silver"
                    }`}
                >
            ...
            <div className="mt-10 grid gap-4">
              ...
              <div className="mt-2">
                <button
                  ...
                  className="text-xs font-mono uppercase tracking-[0.14em] text-noid-silver/70 transition-colors hover:text-white"
                >
                  Request Access
                </button>
              </div>
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <Link
                  ...
                  className="w-full rounded-full border border-noid-silver/25 bg-noid-black/40 px-6 py-3 text-center text-xs font-semibold uppercase tracking-[0.22em] text-white transition-opacity hover:opacity-90"
                >
                  Join Waitlist
                </Link>
                <Link
                  href={"/pilot/apply" as Route}
                  className="w-full rounded-full bg-noid-gold px-6 py-3 text-center text-xs font-semibold uppercase tracking-[0.22em] text-noid-black transition-opacity hover:opacity-95"
                >
                  Apply for Pilot
                </Link>
              </div>
