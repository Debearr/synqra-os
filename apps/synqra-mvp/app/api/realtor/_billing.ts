...
export async function applyStripeSubscriptionCreated(
  userId: string,
  payload: {
    stripeCustomerId: string | null;
    stripeSubscriptionId: string | null;
  ...
  const nowIso = payload.occurredAtIso ?? new Date().toISOString();
  await writeVerticalBillingEntry(userId, vertical, (current) => ({
    ...current,
    tier: "pro",
    ...
    stripe_subscription_status: payload.stripeSubscriptionStatus,
    updated_at: nowIso,
  }));
}
...
export async function applyStripeSubscriptionCancelled(
  ...
  await writeVerticalBillingEntry(userId, vertical, (current) => ({
    ...
    stripe_subscription_id: payload.stripeSubscriptionId,
    stripe_price_id: payload.stripePriceId,
    stripe_subscription_status: payload.stripeSubscriptionStatus ?? "canceled",
    updated_at: nowIso,
  }));
}
