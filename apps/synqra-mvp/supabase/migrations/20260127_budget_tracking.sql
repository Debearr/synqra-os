...
create type if not exists throttling_state as enum (
  'NORMAL',
  'ALERT',
  'CACHE_EXTENDED',
  'D1_DISABLED',
...
begin
  ...
  v_percentage := (v_usage / v_limit) * 100;
...
insert into public.budget_usage (used, limit, period_start, period_end)
values (
  0,
  100,
  date_trunc('month', now()),
